...
main =
    ...
    in putFigures' $ map (second $ scale 4)
        [ (31, let [z1, z2, z3] = map (polar 0.6 . (+ pi / 2)) [0, 2 * pi / 3, -2 * pi / 3]
               in mconcat $ map (put . fromVertices . map (origin .+^) . flip map [-0.6, 1.6] . uncurry interpolate)
                   [ (z2, z3), (z1, z3), (z1, z2) ]
          )

        , (32, let [z1, z2, z3] = map (polar 0.6 . (+ 3 * pi / 2)) [0, 2 * pi / 3, -2 * pi / 3]
               in mconcat $ map (put . fromVertices . map (origin .+^) . flip map [-0.6, 1.6] . uncurry interpolate)
                   [ (z2, z3), (z1, z3), (z1, z2) ]
          )
        ...
        , (11, let p = 0.8 ; t = 1.2 ; x = -0.5 ; y = -0.5
